...
describe('YearSelector', () => {
  ...
  it('calls onChange when option pressed', async () => {
    ...
    await fireEvent.press(getByText('3M'));
  ...
  it('handles layout measurement', async () => {
    const { getByText } = await renderWithProviders(
      <YearSelector options={OPTIONS} value="1Y" onChange={jest.fn()} />,
    );
    expect(getByText('1Y')).toBeTruthy();
  ...
  it('fires onLayout with valid dimensions and sets measured state', async () => {
    const onChange = jest.fn();
    ...
    await fireEvent(getByTestId('year-selector-container'), 'layout', {
  ...
  it('handles onLayout with zero width gracefully', async () => {
    const { getByTestId } = await renderWithProviders(
      <YearSelector options={OPTIONS} value="1Y" onChange={jest.fn()} />,
    );
  ...
  it('animates slider when value prop changes after layout measurement', async () => {
    const onChange = jest.fn();
    ...
    await rerender(<YearSelector options={OPTIONS} value="3M" onChange={onChange} />);

    expect(getByTestId('year-selector-container')).toBeTruthy();
  });

  it('does not animate when value is not in options after measurement', async () => {
    const onChange = jest.fn();
    const { getByTestId, rerender } = await renderWithProviders(
      <YearSelector options={OPTIONS} value="1Y" onChange={onChange} />,
    );
    ...
    await rerender(<YearSelector options={OPTIONS} value="INVALID" onChange={onChange} />);
