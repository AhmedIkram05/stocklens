...
flowchart LR
    ...
    subgraph AWS["AWS eu-west-2"]
        ...
        subgraph Brain["Intelligence"]
            ...
            RUST["Rust/PyO3 features<br/>13 indicators · 4 derived"]
...
| **ML Model**          | PyTorch Global LSTM (entity embeddings) + 5-seed probability ensemble + Optuna HPO (30 trials, per-epoch pruning) | 17 features, ~100 live tickers × 10yr OHLCV (7,389-ticker/28.2M-row DB)                                                          |
...
| **MLOps**             | Airflow weekly retraining, Evidently AI drift detection, statistically-gated champion/challenger promotion        | paired McNemar test on re-scored champion (>2pp + p<0.05), PSI/KS/JSD thresholds, MLflow tracking, S3 delivery                   |
...
| **Weekly champion/challenger gate**           | Airflow (Monday 06:00 UTC): Rust feature engine → Optuna HPO (30 trials, per-epoch pruning) → promote the challenger (a 5-seed probability ensemble) only if directional accuracy improves > 2 percentage points **and** the gain is statistically significant — the champion is re-scored on the challenger's exact test set and compared via an exact two-sided McNemar test (paired statistics, `ml/promotion_stats.py`) → Evidently PSI/KS/JSD drift reports to S3. The full retrain loop is automated - not a notebook, not a Lambda cron. |
...
> Retrofitted a read-only GraphQL layer onto a 70+ endpoint FastAPI REST API: schema-first Strawberry design; batch dataloaders eliminating N+1; JWT resolver-level authz; graphql-codegen typed RN client. PortfolioDetail reads one typed query + a quote stream replacing a 30s blind poll (graphql-ws over RN WebSocket; one shared 60s poller + Redis pub/sub feeds N viewers; ~15-min-delayed data framed as near-real-time).

One typed read graph for the RN app (schema committed at `schema.graphql`, types generated, no live introspection); REST stays the command facade — writes, OCR, uploads, and every other screen are unchanged. Yahoo data is ~15-min delayed and polled every 60s: the stream is **near-real-time, never "live"**. Schema sync rule: any `src/graphql/schema.py` change → regenerate + recommit `schema.graphql` (drift check in [deep dives](docs/deep-dives.md#graphql-read-facade)).
...
| ML model            | Global LSTM: 2 layers (hidden=112, dropout=0.45), 17 features, 5-seed prob ensemble, 30 Optuna trials (per-epoch pruning)            |
| Model performance   | Directional accuracy **53.17%** vs 50% directional chance · +2.3pp over HGB baseline · real-return long-only Sharpe **~2.9 @ 10bps** |
...
| **Focal loss for classification**           | Cross-entropy, weighted CE                 | Focal loss (γ=1.19, tuned via Optuna; pt computed from unweighted CE so class weights don't skew the focal factor) emphasizes hard misclassifications in imbalanced market regimes |
...
# or: push to main → GitHub Actions OIDC → 8-job, 5-stage deploy (no AWS keys in GitHub)
```
