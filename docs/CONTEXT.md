...
| **Global Model** | A single LSTM trained on ~100 live US-listed tickers simultaneously (recency-filtered from a 7,389-ticker / 28.2M-row OHLCV DB), with ticker entity embeddings to learn ticker-specific patterns. Generalizes to unseen tickers via UNK. | One model family with one MLflow run. Not per-ticker models. Train set = ~102 recency-filtered live tickers (`TRAINING_TICKERS=ALL`, 90-day recency filter) across a 10-year lookback (`ML_OHLCV_YEARS`). |
...
| **Technical Indicators** | The feature set computed from raw OHLCV for each ticker: 17 features — 13 via Rust engine (log returns 1/5/21d, scale-free MAs log(close/MA) 5/10/20/50, RSI(14), MACD line/signal/hist ÷close, vol_30d, vol_rank), vol_pct (**causal expanding-rank percentile**, min 60 periods, identical helper at train & serve), and 3 cross-sectional excess returns vs SPY (excess_ret_1d/5d/21d). Extra indicators exist in Rust engine (BB, ATR, OBV, Williams %R, ROC) but are dropped in features.py. | All computed from `adjusted_close`. Cross-sectional features require SPY benchmark data in `ohlcv_prices` table (pipeline aborts without it). Standardised via **global pooled z-score** (one set of means/stds across all tickers, fit on train only). NaN-feature windows filtered out. |
| **Log Returns** | `ln(P_t / P_{t-1})` — the standard transform for financial time series. Used as the primary return metric for labeling, feature engineering, and Sharpe evaluation. | Daily log returns. Multi-period: 1d, 5d, 21d log returns included as features. |
| **Adaptive Label** | The UP/FLAT/DOWN label computed using a rolling volatility threshold, not a fixed percentage. 5-day forecast horizon (forward return over 5 trading days). | `FLAT` if \|log_return\| < threshold_mult × σ_30d × sqrt(horizon). `UP`/`DOWN` otherwise. `σ_30d` = rolling 30-day std of daily log returns. `threshold_mult=2.0` (`ML_THRESHOLD_MULT`, selected on validation — never test; ~73% of labels are FLAT so directional accuracy is the headline metric). `sqrt(horizon)` scaling keeps class balance stable as horizon changes. |
...
| **Directional Accuracy** | Percentage of correct direction predictions across all test samples where the true label is UP or DOWN. Not just UP accuracy. | Computed on the held-out test set (last 15% chronological, 10-day embargo). **Chance level = 50%** (binary-style metric) — not 33%. At threshold 2.0, ~73% of labels are FLAT, so 3-class accuracy is low by design (abstention). |
| **Real-Return Sharpe** | Sharpe ratio from actual forward log returns: long-only (UP predictions) or long-short (±1 on UP/DOWN), evaluated on non-overlapping stride-5 windows per ticker, portfolio returns equal-weighted per date, annualised ×√(252/5). | `Sharpe = mean(portfolio_return) / std(portfolio_return) × sqrt(252/h)`. Reported at 0bps and 10bps round-trip costs. Replaces the retired "Simulated Sharpe" that used a fake ±1% label proxy. |
| **Long-Short Sharpe** | Long-short variant of Real-Return Sharpe: long UP predictions, short DOWN predictions, flat on FLAT. | Same stride-5 per-date equal-weight portfolio construction; cost variants 0bps/10bps. At 10bps the long-short edge ≈ 0 — reported honestly. |
...
| **Model Quality Limitations** | Rebuilt pipeline: directional accuracy 53.17% (5-seed ensemble, n=2,221, chance = 50%) vs HGB baseline 50.83% and logistic 49.06% on identical splits. At threshold 2.0 the model never predicts FLAT at margin 0 (3-class accuracy ~3.5% — abstention trade-off, directional accuracy is the headline). Long-short Sharpe ≈ 0 at 10bps costs. | Financial time series has inherently low SNR. Known remaining work: walk-forward/multi-fold evaluation (single split), binary-collapse ablation, V2 Rust features retest, prod ECS env wiring for full-universe retraining. |

### Data Flow — Prediction

```
Client → GET /predict/{ticker}
  → FastAPI router → prediction_service (lifespan-loaded 5-seed GlobalLSTM ensemble)
  → Fetch PREDICTION_FETCH_LIMIT days of OHLCV from market/repository.py (DB cache; deep window so the causal vol_pct expanding rank matches training)
  → Compute technical indicators (30-day window × 17 features, same causal vol_pct helper as training)
  → Convert to tensor → ensemble forward pass (softmax probs averaged over seeds) → margin rule (|p_up − p_down| < margin ⇒ FLAT)
  → Return { ticker, prediction: "UP"|"FLAT"|"DOWN", confidence, probabilities, margin }
...
backend/ml/pipeline.py
   1. Fetch OHLCV from ohlcv_prices (10yr lookback, ~102 recency-filtered live tickers; SPY required)
  2. Compute features + labels per ticker (causal vol_pct, scale-free MAs, forward returns)
  3. Build ticker vocabulary → assign embedding indices
  4. Merge all tickers into global dataset with global pooled z-score standardisation (train-fit)
  5. Sort by date globally → chronological split: 70/15/15 with 10-day purge embargo
  6. Create DataLoaders with SequenceDataset
   7. Initialise GlobalLSTM (vocab_size, embed_dim=16, hidden=112, n_layers=2, dropout=0.45, n_features=17)
   8. Train with AdamW + CosineAnnealingLR + FocalLoss (pt from unweighted CE, α=class_weights) + gradient clipping (max_norm=5.0)
   9. Early stopping (patience 15) on smoothed validation directional accuracy (mean of last 3 epochs)
  10. Per seed: evaluate on test set → directional accuracy, coverage, per-class F1, confusion matrix, real-return Sharpe (0/10bps)
  11. Average per-seed softmax probabilities → 5-seed ensemble evaluation (ML_SEEDS seeds)
  12. Log everything to MLflow (tracking server, default http://mlflow:5001, env-overridable)
  13. Paired McNemar gate: re-score champion on challenger's test set → promote only if >2pp AND p<0.05
  14. Register promoted ensemble in MLflow Model Registry + persist to model_registry DB + champion checkpoints to disk
```
...
GlobalLSTM(
  ...
  feature_projection: Linear(n_features + embed_dim, hidden_dim=112) → ReLU  // n_features=17 (13 Rust + causal vol_pct + excess_ret_1d/5d/21d)
  lstm: LSTM(hidden_dim=112, hidden_size=112, num_layers=2, dropout=0.45, batch_first=True, bidirectional=False)
  dropout: Dropout(p=0.45)
  classifier: Linear(112, 3) → logits
)
// Production champion = 5-seed probability ensemble (model.pt + model_seed0..4.pt checkpoints)
```
...
| Term                       | Definition                                                                                                                                                                                                                                                                                                                       | Attributes / Constraints                                                                                                                                                                                      |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Weekly Retraining DAG**  | An Airflow Directed Acyclic Graph that runs weekly: fetch new OHLCV → retrain GlobalLSTM (seed ensemble) → evaluate challenger vs champion via paired McNemar gate → promote only if >2pp directional accuracy improvement AND exact two-sided McNemar p<0.05 on the re-scored champion (`ml/promotion_stats.py::decide_promotion_paired`) → log results to MLflow. | Runs via Airflow LocalExecutor on ECS Fargate (production). Metadata stored in RDS PostgreSQL (no SQLite). EventBridge rule on drift alarm triggers `airflow dags trigger weekly_retraining` via ECS RunTask. NOTE: prod ECS task-def does not yet wire `TRAINING_TICKERS`/`ML_OHLCV_YEARS` env — see TRACKER R14 follow-ups. |
| **Champion Model**         | The best-performing registered model in MLflow (by directional accuracy on validation set). Promoted via MLflow Model Registry aliases.                                                                                                                                                                                          | Stored in `model_registry` DB table with alias='champion'. One champion at a time.                                                                                                                            |
| **Challenger Model**       | The newly-trained model (5-seed probability ensemble) from the weekly retraining DAG. Evaluated against the champion re-scored on the same test set. Promoted only if directional accuracy > champion + 2pp AND exact two-sided McNemar p<0.05 (paired). Fallback: unpaired binomial when no champion checkpoint exists. | Not registered in MLflow until promotion. Evaluated within the same DAG run. Champion re-scoring uses the champion's own means/stds/vocab (vocab remap) on CPU (MPS checkpoint-load limitation). |
| **Promotion Threshold**    | The dual gate for challenger promotion: minimum effect size (2pp directional accuracy improvement) plus statistical significance (exact two-sided McNemar p<0.05 on paired directional decisions, `ml/promotion_stats.py::decide_promotion_paired`). Unpaired `should_promote` binomial remains as documented fallback. | Effect size fixed at 2pp; significance at α=0.05. Metrics computed on the chronological test set (last 15% of data, 10-day embargo); `evaluate()` records `n_directional` / `n_directional_correct` for the test. |
...
Airflow DAG (weekly)
  ...
  │     └── → Existing pipeline: fetch 10yr OHLCV (~102 recency-filtered live tickers) → features → train 5-seed ensemble → evaluate → MLflow
  ├── Task 3: evaluate_challenger → paired McNemar gate (champion re-scored on challenger's test set)
  │     └── Promote only if >2pp directional accuracy AND exact two-sided McNemar p<0.05
...
| Market regime shift (e.g., 2020 COVID crash)     | Training includes crash periods within the 10-year window, so the model learns regime-aware patterns. The 10-year lookback (`ML_OHLCV_YEARS`) naturally rolls regimes over time. |
...
| Real-Return Sharpe is extreme (>5 or < -5)          | Flagged in evaluation report. With real forward returns and stride-5 non-overlapping portfolio construction, extreme values usually mean too few directional windows — check `n_directional`.                                                            |
| Inference latency (model forward pass time)      | ~5-20ms per ticker on CPU with the 112-hidden LSTM (×5 ensemble members per request). Redis cache (6h TTL) prevents repeated inference for same ticker. |
...
| **LLM-as-Judge**                                  | Periodic evaluation of live agent responses using a stronger LLM (Claude Opus 4.8 evaluated against the agent's DeepSeek V3.1). Two paths: (a) live-traffic sampling logs a LangSmith feedback point keyed to the trace; (b) the offline LangSmith experiment runner (`agent_eval/run_experiment.py`) scores golden-set predictions on correctness + relevance via a `ChatBedrockConverse` judge. | Sampling rate: configurable (default 10% of conversations, `AGENT_EVAL_SAMPLE_RATE`). No RDS table — scores stored natively in LangSmith per trace. Judge model: `AGENT_JUDGE_MODEL_ID` (Claude Opus 4.8, Converse API, eu-west-2). No impact on response latency. |
...
| **Judge Criteria**                                | The offline experiment runner scores each golden-set prediction on two criteria via the LLM judge: **correctness** (answer grounded in tool data, no hallucinated numbers) and **relevance** (response directly answers the question). Each scored 0–1.                                                                                                                                           | Defined in `agent_eval/run_experiment.py::_score_criterion`. Replaces the frozen plan's 3-dimension composite (Answer Relevance / Tool Selection / Context Adherence) — LangSmith stores per-criterion scores natively, no composite table.                        |
...
| **Agent Monitoring — LLM Response Relevance**     | Periodic LLM-as-Judge feedback point sampled from live traffic, linked to the conversation's LangSmith trace. See LLM-as-Judge, Judge Criteria.                                                                                                                                                                                                                                                   | Sampling rate configurable via `AGENT_EVAL_SAMPLE_RATE` (default 0.1). Logged via LangSmith `create_feedback` (key `sampled_eval`, score 1.0), not an RDS table.                                                                                                   |
...
```
Live-traffic sampling (after agent response, fire-and-forget):
  → If sampled (random < AGENT_EVAL_SAMPLE_RATE):
    → LangSmith create_feedback(key="sampled_eval", score=1.0, feedback_source_type="app")
    → Linked to the conversation's LangSmith trace. No RDS write, no judge call at runtime.

Offline experiment runner (agent_eval/run_experiment.py, CI or manual):
  → Load golden dataset (agent_eval/golden_dataset.json) into LangSmith
  → For each example: _target runs graph with [SystemMessage(PERSONA_PROMPT), HumanMessage(question)], user_id="eval"
  → Judge (ChatBedrockConverse, AGENT_JUDGE_MODEL_ID = Claude Opus 4.8) scores correctness + relevance (0–1 each)
  → evaluate() records per-criterion scores per trace natively in LangSmith
```
